...
class ChatCell: UICollectionViewCell {
    ...
    var imgUrl:String! {
        didSet {
            if imgUrl != nil {
                ...
                SDWebImageManager.shared().imageDownloader?.downloadImage(with: URL(string: secureUrl), options: SDWebImageDownloaderOptions.continueInBackground, progress: nil, completed: { [weak self] (image, data, error, finished) in
                    if let s = self {
                        if let img = image {
    ...
    var typing: Bool! {
        didSet {
            if typing == true {
                ...
                self.typingView.image = OLImage(data: data!)
    ...
    private let typingView = OLImageView(frame: CGRect.zero)
